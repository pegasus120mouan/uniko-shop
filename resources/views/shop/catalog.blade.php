      ...
      @forelse ($products as $p)
        <div class="group rounded-3xl border border-zinc-200 bg-white p-4 hover:shadow-sm transition">
          ...
          <div class="mt-4 flex items-center justify-between">
            <div class="text-base font-semibold">{{ number_format((float) $p->price, 0, ',', ' ') }} FCFA</div>
            <form method="POST" action="{{ route('cart.add', $p) }}">
              @csrf
              <input type="hidden" name="qty" value="1" />
              <button class="rounded-xl bg-zinc-900 px-3.5 py-2 text-white text-sm font-medium hover:bg-zinc-800" type="submit">Ajouter</button>
            </form>
          </div>
        </div>
      @empty
